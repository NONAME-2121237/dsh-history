...
`;
		...
		function TimelineOverlay(props) {
			...
			(0, react.useEffect)(() => {
				...
				if (port === null) {
					for (let node = el; node !== null; node = node.parentElement) if (node.hasAttribute && node.hasAttribute("data-composer-seat") && node.parentElement !== null) {
				...
				const update = () => {
					const r = (port ?? el.parentElement ?? el).getBoundingClientRect();
				...
				const onScroll = () => {
					...
					raf = requestAnimationFrame(() => {
						...
						if (best !== null) {
							...
							if (seq !== void 0) setActiveSeq(seq);
						}
					});
				};
