		...
		function TimelineOverlay(props) {
			...
			(0, react.useEffect)(() => {
				...
				const onScroll = () => {
					...
					raf = requestAnimationFrame(() => {
						...
						for (const r of port.querySelectorAll("[data-chat-anchor-key]")) {
			...
			const jumpToTurn = (turn) => {
				...
				if (key === void 0 && idx >= 0) {
					const seen = /* @__PURE__ */ new Set();
					let count = 0;
					for (const r of document.querySelectorAll("[data-chat-anchor-key]")) {
						const m = /^(\d+):([a-z-]+)/.exec(r.dataset.chatAnchorKey ?? "");
						...
						seen.add(n);
						if (count === idx) {
							key = r.dataset.chatAnchorKey ?? void 0;
							break;
						}
						count++;
					}
